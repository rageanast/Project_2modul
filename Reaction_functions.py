...
def substitution(equation, f = devision_into_parts):
    ...
    try:
        ...
        if (elem0[1] in halogene_strength) and (elem1[0] in halogene_strength):
            if halogene_strength.index(elem0[1]) < halogene_strength.index(elem1[0]):
                reaction = equation + '-->' + reagents[0].replace(elem0[1], elem1[0])+ ' + ' + reagents[1].replace(elem1[0], elem0[1])
                return balance_equation(reaction), 'замещение'
            else:
                return ''
        return ''
