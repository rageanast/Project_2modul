...
def count_result(equation):
    '''Returns balanced reaction equation or message about the impossibility of responding
    ...
    :rtype: str'''
    result = ''
    potential_result = combustion(equation)
    if potential_result != '':
        result += str(potential_result)
    potential_result = compound(equation)
    if potential_result != '':
        result += str(potential_result)
    potential_result = neutralization(equation)
    if potential_result != '':
        result += str(potential_result)
    potential_result = substitution(equation)
    if potential_result != '':
        result += str(potential_result)
    potential_result = exchange(equation)
    if potential_result != '':
        result += str(potential_result)
    potential_result = hydrolisys(equation)
    if potential_result != '':
        result += str(potential_result)
    try:
        clear()
    except BaseException:
        print("Функция clear вызвана вне приложения")
    if result == "":
        result += "Реакция невозможна"
    return result
...
if __name__ == '__main__':
    ...
    btn64 = Button(window, bg='white', fg='red', text='CE', command=clear)
    ...
    button.pack()
    btn68.place(x=600, y=350, width=50, height=50)
    window.mainloop()
